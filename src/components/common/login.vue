            ...
            <el-col class="login-container" :lg="8" :xs="16" :md="10" :span="16">
                ...
                <div class="bottom">
                    <div class="content">
                        ...
                        <el-form :label-position="'left'" label-width="80px" :model="user" :rules="rules"
                                 ref="loginForm">
                            <el-form-item label="用户名：" prop="username">
                                <el-input v-model="user.username" placeholder="请输入用户名或教师编号"
    ...
    methods: {
        ...
        login() {
            (this.$refs.loginForm as any).validate((valid: boolean) => {
                if (valid) {
                    this.loading = true;
                    request.post('/login', {...this.user}).then((res:any) => {
                        // 根据后端结构调整
                        if (res.code === 200) {
